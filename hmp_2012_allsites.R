...
X <- sampleMetadata %>%
  ...
  filter(!is.na(BMI)) 
table(X$body_site)
table(X$study_condition)
table(X$disease)

x3 <- sampleMetadata |>
  dplyr::filter(study_name=="HMP_2012") |>
  dplyr::filter(!is.na(BMI)) |>
  dplyr::select(where(~ !base::all(base::is.na(.x)))) |>
  returnSamples("relative_abundance",counts=TRUE)

X.datframe <- data.frame(int=1,Age=X$age,Gender=X$gender,BMI=X$BMI)
X.f <- cbind(1,X.datframe$Age,model.matrix(~ X.datframe$Gender)[,-1],X.datframe$BMI)
## numeric 

pseq<-makePhyloseqFromTreeSummarizedExperiment(x3, assay.type = "relative_abundance") 
...
select<-names(which(colSums(otu != 0)>0.05*nrow(otu)))
select<-select[!select=="Other"]

zzz<-colnames(otu)%in%select### Matching
W_count<-as.matrix(data.frame(otu[,zzz],Other=rowSums(otu[,!zzz])))
...
save.image("hmp_2012_allsites.R")
